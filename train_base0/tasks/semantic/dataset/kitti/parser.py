...
class Parser():
  def __init__(self,
    ...
    self.group = self.ARCH["train"]["group"]
  ...
  def update_class_map(self):
    if self.group == 0:
      self.DATA["base"].update(self.DATA["group1"])
      self.DATA["base"].update(self.DATA["group2"])
      self.base = self.DATA["base"]
      self.novel = self.DATA["group0"]
    elif self.group == 1:
      self.DATA["base"].update(self.DATA["group0"])
      self.DATA["base"].update(self.DATA["group2"])
      self.base = self.DATA["base"]
      self.novel = self.DATA["group1"]
    elif self.group == 2:
      self.DATA["base"].update(self.DATA["group0"])
      self.DATA["base"].update(self.DATA["group1"])
      self.base = self.DATA["base"]
      self.novel = self.DATA["group2"]
    else:
      self.DATA["base"].update(self.DATA["group1"])
      self.DATA["base"].update(self.DATA["group2"])
      self.base = self.DATA["base"]
    ...
    # re-id base class
    base_learning_map = {}
    keys = sorted(self.base.keys(), key=lambda d: int(d))
    idx = 1
    for key in keys:
      base_learning_map[key] = idx
      idx += 1

    #reset novel class to 0; update base class
    learning_map_new = self.learning_map.copy()
    for key in self.learning_map.keys():
      if learning_map_new[key] in self.novel.keys():
        learning_map_new[key] = 0
      if learning_map_new[key] in base_learning_map.keys():
        learning_map_new[key] = base_learning_map[learning_map_new[key]]

    #print check info 
    for key in self.learning_map.keys():
      value = learning_map_new[key]
      class_string  ="unlabeled"
      for raw_key in base_learning_map:
        if (value == base_learning_map[raw_key]) and (self.learning_map[key] in self.base):
          class_string = self.base[self.learning_map[key]]
      print(f"{key} -> {value} -> {class_string}")

    keys = np.unique(sorted(learning_map_new.values(), key=lambda d:int(d)))
    learning_map_inv_new = {}
    for key in keys:
      if key not in learning_map_inv_new:
        for k, v in learning_map_new.items():
          if key==v:
            learning_map_inv_new[key] = k
            break

    self.learning_map_inv = learning_map_inv_new
    self.learning_map = learning_map_new
